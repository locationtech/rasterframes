...
class RasterSourceTest(TestEnvironment):
    ...
    def test_list_of_str(self):
        ...
        self.assertTrue(
            len(df.columns) == 2
            and "proj_raster_path" in df.columns
            and "proj_raster" in df.columns
        )
    ...
    def test_list_of_list_of_str(self):
        ...
        self.assertEqual(
            sorted(df.columns),
            sorted(["proj_raster_0_path", "proj_raster_1_path", "proj_raster_0", "proj_raster_1"]),
        )
        uri_df = df.select("proj_raster_0_path", "proj_raster_1_path").distinct()

        # check that various uri's are in the dataframe
        self.assertEqual(
            uri_df.filter(col("proj_raster_0_path") == lit(self.path(1, 1))).count(), 1
        )

        self.assertEqual(
            uri_df.filter(col("proj_raster_0_path") == lit(self.path(1, 1)))
        ...
        self.assertEqual(
            uri_df.filter(col("proj_raster_0_path") == lit(self.path(3, 1)))
    ...
    def test_pandas_source(self):
        ...
        df = self.spark.read.raster(self.path_pandas_df(), catalog_col_names=["b1", "b2", "b3"])
    ...
    def test_catalog_named_arg(self):
        ...
        df = self.spark.read.raster(
            catalog=self.path_pandas_df(), catalog_col_names=["b1", "b2", "b3"]
        )
        self.assertEqual(len(df.columns), 7)  # three path cols, three tile cols, and geo
        self.assertTrue(df.select("b1_path").distinct().count() == 3)
    ...
    def test_spatial_partitioning(self):
        ...
        self.assertEqual(
            df.rdd.getNumPartitions(), int(self.spark.conf.get("spark.sql.shuffle.partitions"))
        )
        self.assertEqual(
            self.spark.read.raster(f, spatial_index_partitions=34).rdd.getNumPartitions(), 34
        )
        self.assertEqual(
            self.spark.read.raster(f, spatial_index_partitions="42").rdd.getNumPartitions(), 42
        )
        self.assertFalse(
            "spatial_index" in self.spark.read.raster(f, spatial_index_partitions=False).columns
        )
        self.assertFalse(
            "spatial_index" in self.spark.read.raster(f, spatial_index_partitions=0).columns
        )
