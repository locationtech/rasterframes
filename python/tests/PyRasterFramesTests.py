...
class TileOps(TestEnvironment):
    def setUp(self):
        ...
        self.t1 = Tile(np.array([[1, 2], [3, 4]]), CellType.int8().with_no_data_value(3))
        self.t2 = Tile(np.array([[1, 2], [3, 4]]), CellType.int8().with_no_data_value(1))
        self.t3 = Tile(np.array([[1, 2], [-3, 4]]), CellType.int8().with_no_data_value(3))
    ...
    def test_subtraction(self):
        ...
        self.assertTrue(r1 == e1, "{} does not equal {}".format(r1, e1))
        # put another way
        self.assertTrue(r1 == self.t1 * 3, "{} does not equal {}".format(r1, self.t1 * 3))

    def test_division(self):
        t3 = self.t1 * 9
        r1 = t3 / 9
        self.assertTrue(
            np.array_equal(r1.cells, self.t1.cells), "{} does not equal {}".format(r1, self.t1)
        )
...
class PandasInterop(TestEnvironment):
    ...
    def test_extended_pandas_ops(self):
        ...
        self.assertTrue(all([isinstance(row.tile.cells, np.ndarray) for row in rf_collect]))
...
class RasterJoin(TestEnvironment):
    ...
    def test_raster_join(self):
        ...
        rf_prime = self.spark.read.geotiff(self.img_uri).withColumnRenamed("tile", "tile2")
        ...
        rf_joined_2 = self.rf.raster_join(
            rf_prime, self.rf.extent, self.rf.crs, rf_prime.extent, rf_prime.crs
        )
        self.assertTrue(rf_joined_2.count(), self.rf.count())
        self.assertTrue(
            len(rf_joined_2.columns) == len(self.rf.columns) + len(rf_prime.columns) - 2
        )
        ...
        rf_joined_3 = self.rf.raster_join(
            rf_prime, self.rf.extent, self.rf.crs, rf_prime.extent, rf_prime.crs, join_expression
        )
        self.assertTrue(rf_joined_3.count(), self.rf.count())
        self.assertTrue(
            len(rf_joined_3.columns) == len(self.rf.columns) + len(rf_prime.columns) - 2
        )
    ...
    def test_raster_join_resample_method(self):
        ...
        df = self.spark.read.raster(
            "file://" + os.path.join(self.resource_dir, "L8-B4-Elkton-VA.tiff")
        ).select(col("proj_raster").alias("tile"))
        df_prime = self.spark.read.raster(
            "file://" + os.path.join(self.resource_dir, "L8-B4-Elkton-VA-4326.tiff")
    ...
    def test_raster_join_with_null_left_head(self):
        ...
        try:
            ...
            self.assertTrue(
                joined_no_left_tile.select(isnull("t")).filter(col("j") == "b").first()[0]
            )
            # Because no non-null tile col on Left side, the right side is null too
            self.assertTrue(
                joined_no_left_tile.select(isnull("r")).filter(col("j") == "b").first()[0]
            )
