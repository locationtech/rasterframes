...
from .rf_types import RasterFrameLayer, RasterSourceUDT, TileExploder, TileUDT
...
def _kryo_init(builder: SparkSession.Builder) -> SparkSession.Builder:
    ...
    builder.config("spark.serializer", "org.apache.spark.serializer.KryoSerializer").config(
...
def _convert_df(df: DataFrame, sp_key=None, metadata=None) -> RasterFrameLayer:
    ...
    if sp_key is None:
        ...
        return RasterFrameLayer(
            ctx._jrfctx.asLayer(df._jdf, _to_java_column(sp_key), json.dumps(metadata)),
            ctx._spark_session,
        )
...
def _raster_join(
    ...
    if join_exprs is not None:
        assert (
            ...
            and left_crs is not None
            and right_extent is not None
            and right_crs is not None
        )
    ...
    **options: str,
    ...
    if catalog is not None:
        ...
        if isinstance(catalog, str):
            ...
            assert all(
                [c in catalog.columns for c in catalog_col_names]
    ...
    return df_reader.format("raster").load(path, **options)


def _stac_api_reader(df_reader: DataFrameReader, uri: str, filters: dict = None) -> DataFrame:
    """
...
def _geotiff_writer(
    ...
    **options: str,
    ...
    def set_dims(parts):
        ...
        options.update({"imageWidth": str(parts[0]), "imageHeight": str(parts[1])})
        parts = [int(p) for p in parts]
...
DataFrameReader.geotrellis_catalog = lambda df_reader, path: _aliased_reader(
    df_reader, "geotrellis-catalog", path
)
