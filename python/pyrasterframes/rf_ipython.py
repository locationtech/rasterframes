...
def plot_tile(
    ...
    **imshow_args,
    ...
    def normalize_cells(cells: np.ndarray) -> np.ndarray:
        ...
        ), "invalid upper and lower percentiles {}, {}".format(lower_percentile, upper_percentile)
...
def tile_to_png(
    ...
    if title is None:
        axis.set_title(
            "{}, {}".format(tile.dimensions(), tile.cell_type.__repr__()), fontsize=fig_size[0] * 4
        )  # compact metadata as title
    else:
...
def pandas_df_to_html(df: DataFrame) -> Optional[str]:
    ...
    )  # we'll try to politely put it back
    ...
    for c in df.columns:
        if isinstance(
            df.iloc[0][c], pyrasterframes.rf_types.Tile
...
def _folium_map_formatter(map) -> str:
    ...
    b64_source = base64.b64encode(bytes(html_source.encode("utf-8"))).decode("utf-8")
...
try:
    ...
    from IPython.display import display, display_html, display_markdown, display_png
