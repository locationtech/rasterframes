...
object MODISCatalogDataSource extends ResourceCacheSupport {
  ...
  private def sceneListFile(start: LocalDate, end: LocalDate, useBlacklist: Boolean): File = {
    logger.info(s"Using '$cacheDir' for scene file cache")
    val basename = File(s"$SHORT_NAME-$start-to-$end.csv")
    cachedFile(basename).getOrElse {
      ...
      for {
        output <- retval.createFile().printWriter()
        input <- inputs
        line <- input.lineIterator.drop(1) // Drop header line
      } output.println(line)
      retval
    }
  }
