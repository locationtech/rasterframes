...
// NB: copied from the private geotrellis.raster.io.geotiff.SegmentTransform

package astraea.spark.rasterframes.ref

import geotrellis.raster.io.geotiff.{GeoTiffSegmentLayout, GeoTiffSegmentLayoutTransform}

trait SegmentTransform {
  def segmentIndex: Int
  def segmentLayoutTransform: GeoTiffSegmentLayoutTransform
  protected def segmentLayout: GeoTiffSegmentLayout
...
case class StripedSegmentTransform(segmentIndex: Int, segmentLayout: GeoTiffSegmentLayout, segmentLayoutTransform: GeoTiffSegmentLayoutTransform) extends SegmentTransform {
  def gridToIndex(col: Int, row: Int): Int = {
...
case class TiledSegmentTransform(segmentIndex: Int, segmentLayout: GeoTiffSegmentLayout, segmentLayoutTransform: GeoTiffSegmentLayoutTransform) extends SegmentTransform {
  def gridToIndex(col: Int, row: Int): Int = {
    val tileCol = col - (layoutCol * tileCols)
    val tileRow = row - (layoutRow * tileRows)
