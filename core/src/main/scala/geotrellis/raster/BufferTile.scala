...
import geotrellis.raster.mapalgebra.focal.BufferedFocalMethods
...
case class BufferTile(
  ...
  require(
    gridBounds.colMin >=0 && gridBounds.rowMin >= 0 && gridBounds.colMax < sourceTile.cols && gridBounds.rowMax < sourceTile.rows,
   ...
   */
  def map(f: Int => Int): BufferTile = mapTile(_.map(f))
   ...
   * @return     The result, a [[Tile]]
   */
  def mapDouble(f: Double => Double): BufferTile = mapTile(_.mapDouble(f))
   ...
   * @return          The result, a [[Tile]]
   */
  def mapIntMapper(mapper: IntTileMapper): BufferTile = mapTile(_.mapIntMapper(mapper))
   ...
   * @return          The result, a [[Tile]]
   */
  def mapDoubleMapper(mapper: DoubleTileMapper): Tile = mapTile(_.mapDoubleMapper(mapper))

  private def combine(other: BufferTile)(f: (Int, Int) => Int): Tile = {
  ...
  def combine(other: Tile)(f: (Int, Int) => Int): Tile = {
    ...
    other match {
      case bt: BufferTile => this.combine(bt)(f)
      case _ =>
        val tile = ArrayTile.alloc(cellType.union(other.cellType), cols, rows)
        cfor(0)(_ < rows, _ + 1) { row =>
          cfor(0)(_ < cols, _ + 1) { col =>
...
object BufferTile {
  implicit class BufferTileOps(val self: BufferTile) extends BufferedFocalMethods
}
