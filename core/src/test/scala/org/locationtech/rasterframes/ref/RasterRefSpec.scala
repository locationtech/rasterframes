...
class RasterRefSpec extends TestEnvironment with TestData {
  ...
  trait Fixture {
    ...
    val subRaster = RasterRef(src, 0, Some(subExtent), Some(src.rasterExtent.gridBoundsFor(subExtent)), 0.toShort)
  }
  ...
  describe("RasterRef creation") {
    it("should realize subiles of proper size") {
      ...
      val dims = src
        ...
        .map(e => RasterRef(src, 0, Some(e), None, 0.toShort))
        .map(_.dimensions)
        .distinct
  ...
  describe("buffering") {
    ...
    val refs = src
      .layoutExtents(NOMINAL_TILE_DIMS)
      .map(e => RasterRef(src, 0, Some(e), None, 3))
      .map(r => RasterRefTile(r))
